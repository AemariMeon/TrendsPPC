...
let cardA = null;
let cardB = null;
let playerAlias = ''; // New variable to store the player's alias
...
function initGame() {
    ...
    // 2. Set up listener for alias submission
...
function getRandomCard(excludeCard = null) {
    let newCard;
    let attempts = 0;
    const maxAttempts = TIKTOK_DATA.length * 2;
    do {
...
window.makeGuess = function(guess) {
    ...
    // Check for a tie (followers are the same, which is possible in the data)
    ...
    if (correct) {
        ...
        if (currentRound < MAX_ROUNDS) {
            currentRound++;
            // Move Card B to Card A, and get a new Card B
            setTimeout(() => {
                cardA = cardB;
                cardB = getRandomCard(cardA);
...
function endGame() {
    ...
    finalScoreEl.innerHTML = `Congratulations, **${playerAlias}**! You completed **${currentRound-1}** rounds with a final streak of **${currentStreak}**!`;
    
    // Show modal and update leaderboard
    setTimeout(() => {
